...
import numpy as np
from kernel_exp_family.estimators.structured_densities.gaussian_with_marginals import compute_submatrix, compute_C, \
    compute_b, compute_allb, compute_allC, compute_alltheta,\
    create_groupstructure, extract_simple_C, extract_simple_b, KernelExpStructuredGaussianMarginals
...
def test_alltheta_correctly_extracted_from_theta_vector():
    ...
    theta2 = np.concatenate([alltheta[0,1,:],alltheta[0,2,:],alltheta[1,2,:]])
...
def test_allb_correctly_extracted_from_b_vector():
    ...
    b2 = np.concatenate([allb[0,1,:],allb[0,2,:],allb[1,2,:]])
    assert np.all(b == b2)
...
def test_oop_interface():
    ...
    lmbda = 1
    m = 100
    est = KernelExpStructuredGaussianMarginals(sigma, lmbda, m)
